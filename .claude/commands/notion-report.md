...
This is the **"I'm pausing this branch" report** — the *mid-project* entry in the **Branch Handoff Reports** database, branch-not-yet-merged, optimized for *resuming*. Its single most important output is a copy-paste `cd` line into this worktree, plus a paste-after-compacting prompt.

> **Note:** `/pr-merge` and `/promote-to-main` no longer write to this database. They report their outcome in the terminal and stop. Nothing closes a handoff page out to `Status = Merged` automatically, so treat a page here as a pause record rather than a branch's full lifecycle.
