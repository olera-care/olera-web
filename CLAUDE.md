...
- The MedJobs sidebar is three entries: **Universities**, **Standard Operating Procedures (SOP)** (which opens `/admin/medjobs/sop/scripts`), and **Archive**. The four role manuals — **System, Admin, Sales, CRM**, all under `/admin/medjobs/sop` — are buttons at the bottom of the Archive page, and stay searchable and pinnable via `SOP_PAGES` in `AdminSidebar.tsx`. System is the operating command centre and carries the Sites and In Basket actions; `/admin/medjobs/stats` still works by URL but is no longer linked. All four documents are served via `/api/admin/medjobs/sop?doc=…`. New or renamed PDFs need an entry in `outputFileTracingIncludes` in `next.config.ts`, or the route 404s in production.
...
1. **Current task state** — what we're building/fixing, the specific bug or feature, reproduction steps
2. **Modified files list** — every file touched in this session
3. **Test status** — which tests pass/fail, commands used, error messages
4. **Architectural decisions** — choices made and rejected alternatives (with reasons)
5. **SCRATCHPAD.md context** — any session notes written there
6. **Git state** — current branch, uncommitted changes, recent commits

Drop: exploration that didn't lead anywhere, large file reads that were just for understanding, superseded approaches.
