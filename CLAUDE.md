...
## Parallel Routes (Texas)

Texas has **separate routes** that shadow the generic `/senior-benefits/` routes via 301 redirects in `next.config.ts`:

| Generic route | Texas route | Redirect |
|---|---|---|
| `/senior-benefits/[state]` | `/texas/benefits` | 301 |
| `/senior-benefits/[state]/[benefit]` | `/texas/benefits/[slug]` | 301 |

**When upgrading any benefits page component (state or program), you MUST update both the generic route AND the Texas-specific route.** The Texas routes are what users actually hit — the generic routes only serve non-Texas states. This has caused V1/V2 rendering bugs twice (Apr 2026 post-mortem).
