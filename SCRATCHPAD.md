...
- **Build status:** PR #916 **Vercel CI = PASS** (~13 commits). Local `tsc` via borrowed desktop node_modules shows 8 errors in UNRELATED files (PasskeysSection/UnifiedAuthModal/program-pdf) — a Supabase-2.106 deps-skew artifact, NOT real; CI green. (NOTE: those 8 are a real pre-existing issue on the branch's desktop checkout, unrelated to this work.)
...
**Next up → PHASE 2 (active; PLAN WITH TJ BEFORE WRITING CODE — his explicit ask 2026-06-03).** Make the diagnostic compute on-demand for any provider/city instead of CS-by-hand: `market_diagnostics` cache table (city/state/care_type/data jsonb/generated_at/status) + a server trigger that runs the engine (`fetch`+`analyze`, ~60-90s, can't block page load) on first visit to an uncached market → background job / queue → API reads table not committed file → then flip the gate for rollout. Open design Qs for the plan: cache-miss UX (background "building your report" + poll/notify vs precompute-on-claim); where the engine runs (Vercel function timeout limits — 60-90s compute may need a cron/queue or chunked steps); how to port the .mjs engine to run server-side (it's dependency-free fetch, but needs the API keys + write to the table). Then (later): review-climb tracking in workspace; verify-attorney-practice-area enrichment (recover elder-law); care-type-adaptive copy (referral % line hardcoded "home care"). Memory: `project_market_diagnostic`, `feedback_dont_over_anchor_offers`, `project_comfort_keepers`.
