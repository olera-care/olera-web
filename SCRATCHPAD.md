...
Pre-test fixed cohort-mismatched Q&A response rates, historical growth rows looking fresh, missing GA4/GSC values in the AI pack, duplicate paid runs across tabs, rapid-range/client races, evidence mislabeling and redaction, and company-revenue overclaiming. **Files:** `app/admin/war-room`, `app/api/admin/war-room`, `components/admin/WarRoomDashboard.tsx`, `lib/war-room/*`, the admin sidebar, `.env.example`, and migration 176. **Validation:** TypeScript, targeted ESLint, diff checks, route/scenario checks, the cron registry, and Next compilation pass; full static generation remains blocked locally by absent Supabase environment variables. **Next:** apply migration 176, preview one 30-day Opus run and decision lifecycle, then merge only after TJ approval.
