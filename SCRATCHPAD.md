...
Moved Olera's weekly growth reporting out of the separate Olera-HQ/Airtable workflow and into Olera Web. Added one shared TypeScript collector for GA4, Search Console, and same-week Supabase outcomes; an immutable `growth_metric_snapshots` table; Tuesday Vercel cron; authenticated admin API; maintenance-only `/metrics`; gap-safe backfill; one-time Airtable CSV importer; and a new 12-week Growth view above the existing Product signals analytics. Definition v2 adds a passive Organic acquisition panel with GA4 source/medium, organic landing pages, Search Console search health, top queries, and an honestly labeled branded/non-branded mix. Corrected the old headline from generic “page views” to “provider profile views” and collapsed Ad Boost by default.
...
**Files:** growth schema/collector/store/date types, cron + registry, admin Growth API/UI, `/metrics`, Airtable importer, growth docs, package scripts, Vercel schedule, Claude/Codex guidance. **Validation:** live GA4/GSC/Supabase pull and forced saved-row readback, clean path-only landing-page audit, valid/invalid Airtable fixture dry-run, `tsc --noEmit`, 32-job cron registry, diff check, and full production build pass. **PR:** #1557 against `staging`. **Next:** preview-QA `/admin/analytics`, then export/import Airtable history and run gap backfill. Do not merge without TJ.
