...
**Next:** apply migration `168_sms_drafts.sql` in the Supabase dashboard (until then autosave shows "Draft not saved" in red; everything else is unchanged), then **send her the reply** — the drafted text is on the Notion card *LA care seeker stuck on hold*, and the one thing we still need from her is her **parish**, which unlocks the direct COA number.
...
- 🔴 **Send the LA family her reply, and get her parish.** Profile `53ac7f4e`, the first family ever to answer the check-in. Draft is on the Notion card *LA care seeker stuck on hold*. Her parish is the one missing field (profile is 43% complete, no city or zip) and it unlocks the direct Council on Aging number for the voucher she was actually looking for. Text, do not email — her `preferred_contact_channel` says email and that is not where she lives.
- 🔴 **Apply migration `168_sms_drafts.sql`** in the Supabase dashboard before QAing the inbox drafts feature (`calm-`). The inbox degrades gracefully until then, but autosave shows "Draft not saved" in red.
