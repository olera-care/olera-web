...
### 2026-05-11 (Sun, late) — Honest sitemap `<lastmod>` (the useful tail of Project 6 Step 4) + the de-indexing batch all shipped to prod

After the 21K description rewrite, the question came up: should we resubmit the sitemap? Dug in — a bare GSC "resubmit" is ~a no-op (Google re-fetches the file, which it does anyway, and the file looks identical because **every provider URL was emitted with `lastModified: new Date()` on every fetch** — for all ~75K, every time — so Google long ago learned our `<lastmod>` is noise and ignores it). The *real* move, exactly now: make `lastmod` honest so Google prioritizes recrawling the ~21K pages we just rewrote. (Pruning the ~200 zero-signal pages = still a no-op; the bigger lever — internal linking, since the thin pages are also the most internally-orphaned — is a real project, **deliberately deferred** ("kick that"), not done.)

**Shipped (PR #786 → staging `b7334fff`):**
- `app/sitemap.ts` + `app/api/sitemap/route.ts` — provider URLs now emit `lastmod = COALESCE(description_updated_at, cleansed_at, created_at)`. Each shard *probes* for the columns first and degrades to the old behavior (minimal select + `new Date()`) if the migration isn't applied — so shipping the code before/after the migration is both safe (no broken shard; verified the 400→fallback path against prod).
- Migration `082_provider_description_updated_at.sql` — adds nullable `description_updated_at` to `olera-providers`, backfilled `= now()` for every row with a `provider_description_v1_backup`. **Applied by TJ via the Supabase dashboard 2026-05-11** — verified: column exists, 21,166 active rows stamped (matches the v1_backup count exactly).
- `scripts/backfill-provider-descriptions.js` — now stamps `description_updated_at` on every rewrite. New providers from the city pipeline leave it NULL → sitemap falls back to `created_at` (correct "last updated" for a fresh listing).
- *Not touched (intentional):* power-page / static-page `lastModified` (`new Date()` defensible for aggregations), `business_profiles` (didn't risk a schema assumption). *Flagged for later:* `app/api/sitemap/route.ts` is a redundant second sitemap not referenced in `robots.txt` — candidate for deletion.

**Whole de-indexing batch is now in production** (`main` at `c9629f24` via PR #774's staging→main promotion + PR #786 still to be promoted): review-page `noindex` (#771), deleted-provider 301s + removal-blocklist (#764/#766), 21K regenerated descriptions (data live via shared DB), the Vercel firewall fix (dashboard, live), and — after the next promotion — honest sitemap `lastmod` (#786). Team Slack posted to #ai-product-development. Memory added: `feedback_perplexity_real_cost.md` (Sonar is ~$8/1000, not $1/1000 — the script comment is token-cost-only; use `--engine claude` Haiku ~$1.25/1000 for bulk text gen).

**Next (TJ's, not urgent):** (1) promote `staging` → `main` to carry PR #786 to prod; (2) ~May 18–19, once GSC Host status shows "server connectivity" ≈ 0% (firewall fix confirmed), resubmit `sitemap.xml` in Search Console — *then* it's meaningful; (3) the 4–8 week GSC watch ("not indexed" shrinks, "indexed" climbs off the flatline, 404 crawl-share drops). Realistic forecast: index ratio recovers ~42% → ~55–65%, not 90% — the genuinely-bare provider pages stay out, by design. **Deferred future project:** internal-linking uplift for the orphaned thin-provider long tail (bigger durable lever than any sitemap tweak; has a "creates more thin pages" trap, so deserves its own scoping pass).

---

### 2026-05-11 (Sun, evening) — Project 6 executed: 21,064 boilerplate provider descriptions regenerated; the "thin /provider/" bucket reframed

Ran Project 6 steps 1–3 end-to-end. **Step 1 (quantify) reframed the whole project:** the bucket isn't ~18K "empty" pages — it's templated *prose*. Of 75,352 active providers, 21,064 (28%) had the Gen-1 fill-in-the-blank description ("X provides Y services for senior elders in the City, ST area. To find the right care for you loved one, connect with…"), and 47,687 (64%) carry *some* machine template (Gen-1 + the Gen-2 "X is a [category descriptor] in City. The community is rated N out of 5 stars on Google."). Only ~36% have genuinely unique prose. The strict "zero-signal, prune-from-sitemap" set is ~24–216 — i.e. **Step 4 (sitemap quality threshold) as originally scoped is a near-no-op; skip it or do a minor `priority` tweak.** Backfill backlogs found: ~9K non-CMS providers missing `ai_trust_signals` (small, ~$10 to fill); **~28K CMS-category providers (HHC + NH) missing `cms_data`** — much bigger, the existing backfill script doesn't do CMS imports, flagged as separate future work (a Medicare-Compare ingestion job).

**Steps 2–3 (data backfill + description regen):**
- New `scripts/backfill-provider-descriptions.js` — rewrites Gen-1 templated descriptions into unique, factual prose, grounded in data we already hold (category, location, Google rating, review snippets, verified entity summary, tenure). Dry-run by default; backs up original to `provider_description_v1_backup` only when NULL; no-overwrite-on-failure; resumable (re-queries rows still matching the template signature, so a rewritten row drops out); rejects model output that echoes the template / includes URLs/CTAs / is too short or long. Prompt guardrails after two dry-run iterations: no street addresses, license numbers, survey or founding dates, age cutoffs, prices/bed counts; no naming individuals; **no negative-review themes** (mixed/critical reviews → it drops the review sentence); rating mentioned only if ≥4.0★ with ≥5 reviews (matches the highlights house style).
- **Result: all 21,064 Gen-1 descriptions rewritten** (verified in DB: 0 boilerplate-signature rows remain among active providers). Provider pages pick up the new copy on next ISR revalidation (1 hr).
- Trust-signal pass (`scripts/backfill-highlights-data.js --run --trust-only`): partial — 2,855 of 8,823 non-CMS providers got `ai_trust_signals`, **288 junk entities soft-deleted** (medical-supply shops, ghost listings, a mobile-home park, an under-60 disability facility — good de-indexing hygiene), then stopped (see cost note). The 2,855 we got are fine; TJ said skip the rest.
- Also fixed: a `sanitizeText()` step in the regen script — two provider rows had unpaired UTF-16 surrogates (corrupted import) that made both the Anthropic and Perplexity request bodies invalid JSON ("no low surrogate in string").

**Cost overrun + the fix:** I quoted "~$21 + ~$10" off the `$1 per 1000 requests` comment baked into the scripts. **That's token cost only — Perplexity Sonar's real cost is the ~$8/1000 search/request fee.** The first Perplexity run spent **~$150** (rewrote ~15K of 21K descriptions + the partial trust-signal pass, ~18.2K Sonar calls) before TJ flagged three $50 charges and I killed both jobs. Fix: added an `--engine claude` backend to the regen script (`claude-haiku-4-5`, no web search, ~$1.25/1000 — the model the repo already uses elsewhere), which finished the remaining ~5,874 descriptions for **~$5.87** in 32 min. **Total spend ~$156.** The ~15K Perplexity-written descriptions are good (TJ reviewed samples, approved) and Haiku's output is comparable/cleaner. Lesson saved to memory: check the provider's actual pricing page; don't trust the script's cost constant.

**Commits** (on `scratchpad/eod-2026-05-08-evening`, PR #770): `27d928e8` (regen script), `9daca4d0` (`--engine claude`), `0a8ffbdc` (sanitizer). The script should get cherry-picked onto a proper `feature/*` branch → `staging` if/when we touch `app/sitemap.ts` for Step 4 (or just merge it standalone — it's a one-off tool, not deployed code).

**Project 6 status:** ✅ Step 1 (quantify/reframe). ✅ Step 3 (description regen — complete). 🟡 Step 2 (trust signals — 2,855/8,823, intentionally stopped). 🟡 Step 4 (sitemap — reframed to near-no-op; skip or `priority` tweak only). ⏳ Step 5 (measure in GSC, 30–90d). Open option: finish the trust-signal + the ~28K CMS-data backlog on a cheap engine if highlights coverage becomes a priority — not urgent.
