...
**Growth drivers in progress:** Added migration 173 and a relational weekly page-performance layer for canonical Provider, Benefits, and Editorial destinations. The collector now retains category-scoped GA4 Organic Search users/sessions plus Search Console clicks/impressions/CTR/position; the authenticated admin API aggregates the selected range and its preceding equivalent period. The Organic Growth page gains progressive driver cards, category sparklines, a single Top/Rising/Falling/Opportunities workspace, humanized page labels, direct page links, and deterministic “Nearly there,” CTR, momentum, and new-entrant prompts. The old generic landing list was removed so this consolidates rather than overlaps. **Setup remaining:** apply migration 173 and run `npm run metrics:backfill-pages -- --force` once for the nine existing Google-backed weeks; Airtable-only history cannot provide page rows.

**Files:** growth schema/collector/store/date types, cron + registry, dedicated Organic Growth admin API/UI, `/metrics`, Airtable importer, growth docs, package scripts, Vercel schedule, Claude/Codex guidance. **Validation:** 171-row Airtable dry-run and import, eight-week API gap backfill, final 180-row continuity/source/latest-row audit, clean path-only landing-page audit, `tsc --noEmit`, 32-job cron registry, diff check, and full production build pass. **PR:** #1557 against `staging`. **Next:** preview-QA `/admin/organic-growth` and confirm `/admin/analytics` is focused again. Do not rerun the historical import; the Tuesday cron owns new weeks. Do not merge without TJ.
