...
- 🟡 **Email responder parity.** `classify.server.ts` calls `buildAnswerPacket` on `category: care_seeker`, reuse `AnswerPacketPanel`, relax its eligibility guardrail now that sourcing is enforced. The wiring is a day; **the triage gate is the design problem** — 899 threads at five Opus stages is a different cost conversation than one text a day.
