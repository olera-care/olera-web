...
| 2026-08-21 | Prune the 129 dead allowlist entries rather than wait for the code fix to do it | Measured honestly, PR #1658 only refuses *new* trusts and delivers 2.28% → 2.21% over 90 days. The standing bleed is the entries already on the list, worth 282 bounces and 2.28% → 1.53%. The prune needs no deploy, loses no reach (all 129 had bounced and never once delivered), touches zero claimed accounts, and is reversible from a backup written before the delete. Promoting #1658 remains worth doing, but as the thing that stops the list refilling, not as the fix. |
| 2026-08-21 | Set `hello@seniorlistings.net` from_name to "Logan from Olera" | Providers saw "TJ Falohun" with no company name and an unrecognised domain, which is exactly the "flagged as sketchy" complaint. One field, applies immediately to live campaigns, no warmup. Accepted tradeoff, flagged to TJ before making it: replies still land in `hello@seniorlistings.net` while the name now says Logan, so if the follow-up caller is not Logan this trades one continuity mismatch for another. |
...
**Third thing, small but live today.** Changed `hello@seniorlistings.net`'s SmartLead `from_name` from "TJ Falohun" to **"Logan from Olera"**. The word Olera previously appeared nowhere in what a provider saw, which is complaint #1 in Esther's thread almost verbatim. This is the only piece of that thread that lands immediately rather than after warmup, since it applies to the six live TX campaigns with no deploy and no waiting. Told Esther about the second sender in `#product-development`.
