...
interface Zcta { zcta: string; population: number; seniors65plus: number; medianIncome: number | null }
interface Leader { name: string; reviews: number; rating: number | null; distanceMiles: number | null; website: boolean; shareOfVoicePct: number }
interface BdTarget { name: string; cat: string; referralValue: string; distanceMiles: number | null; reviews: number; rating: number | null; phone: string | null; website: string | null; address: string }
interface Analysis {
...
function Section({ kicker, title, children }: { kicker: string; title: string; children: ReactNode }) {
  return (
    <section className="border-t border-stone-200/80 pt-10 mt-10 first:border-0 first:pt-0 first:mt-0">
...
export default function MarketDiagnosticPage() {
  ...
  return (
    <div className="max-w-3xl">
      {/* Header */}
      <Eyebrow>Client acquisition · {a.meta.city}, {a.meta.state} · {a.meta.careType === "homecare" ? "Home care" : "Assisted living"}</Eyebrow>
      <h1 className="font-display text-[2.5rem] leading-[1.1] text-stone-900 mt-2">
        Where your next clients are
      </h1>
        ...
        <div className="grid grid-cols-3 gap-5">
          ...
          <div>
            <div className="font-display text-[2.75rem] leading-none text-stone-900">
              {dem.medianIncomeRange ? `${usd(Math.round(dem.medianIncomeRange.min / 1000) * 1000).replace(",000", "k")}–${usd(Math.round(dem.medianIncomeRange.max / 1000) * 1000).replace(",000", "k")}` : "—"}
            </div>
      ...
      <Section kicker="Where to focus" title="Not all of town is your customer">
        <p className="text-[14px] text-stone-600 leading-relaxed mb-5 max-w-xl">
          Private-pay home care lives where senior density meets income. These are the areas worth your
      ...
      {/* Referral map — the killer insight */}
      <Section kicker="The unlock" title="The referral map most agencies never build">
        ...
        <div className="space-y-2">
          {ref.prioritizedTargets.filter((t) => t.cat !== "financial").slice(0, 8).map((t) => (
