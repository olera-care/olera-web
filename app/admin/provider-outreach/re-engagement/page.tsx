...
interface BottomFunnelProvider {
  ...
  mail_status?: "draft" | "ready" | "printed" | "in_transit" | "delivered" | "returned" | "cancelled" | null;
  // Tier
  tier?: ProviderTier;
            ...
            <div className="flex items-center justify-between py-1.5">
              <div className="flex items-center gap-2">
                <span className={`w-2 h-2 rounded-full ${
                  analytics.status === "in_transit" || analytics.status === "delivered" || analytics.status === "printed" ? "bg-emerald-500" : "bg-amber-400"
                }`} />
...
export default function ReEngagementPage() {
  ...
  const loadProviders = useCallback(async () => {
    ...
    try {
      ...
      // using their persisted re_engage_channel (default to "re_engage").
      setProviderTabMap(() => {
        const next = new Map<string, BottomFunnelTab>();
        for (const p of apiProviders) {
          const channel = (p.re_engage_channel || "re_engage") as BottomFunnelTab;
      ...
      console.error("[ReEngagementPage] Failed to load providers:", err);
      setProviders([]);
    } finally {
      setLoading(false);
    }
