...
export default function AdminReviewRequestsPage() {
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900">Review Request Engagement</h1>
        <p className="text-lg text-gray-600 mt-1">
          Track which providers are using the review request feature.
        </p>
      </div>
      ...
      {summary && !loading && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-4">
            <p className="text-sm text-gray-500">Providers Using</p>
            <p className="text-2xl font-bold text-gray-900">{summary.total_providers.toLocaleString()}</p>
          </div>
          <div className="bg-white rounded-xl border border-gray-200 p-4">
            <p className="text-sm text-gray-500">This Month</p>
            <p className="text-2xl font-bold text-gray-900">{summary.requests_this_month.toLocaleString()}</p>
          </div>
          <div className="bg-white rounded-xl border border-gray-200 p-4">
            <p className="text-sm text-gray-500">Via Email</p>
            <p className="text-2xl font-bold text-gray-900">{emailPercent}%</p>
          </div>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-8 text-center">
          <p className="text-gray-500">No providers have sent review requests yet.</p>
        </div>
      ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-200 bg-gray-50">
                  ...
                  <th className="text-left px-6 py-3 text-sm font-medium text-gray-500">Requests</th>
                  <th className="text-left px-6 py-3 text-sm font-medium text-gray-500">This Month</th>
                  <th className="text-left px-6 py-3 text-sm font-medium text-gray-500">Olera Reviews</th>
                  <th className="text-left px-6 py-3 text-sm font-medium text-gray-500">Google</th>
