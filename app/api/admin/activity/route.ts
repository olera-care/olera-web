...
async function handleFeedView(db: any, opts: {
  ...
  if (emailType) {
    ...
    } else if (["contact_revealed", "one_click_access", "lead_opened", "email_click", "question_responded", "lead_received"].includes(emailType)) {
...
async function handleProvidersView(db: any, opts: {
  ...
  if (emailType) {
    if (emailType === "suspicious_claim") {
      query = query
        .or("event_type.eq.suspicious_claim,and(event_type.eq.one_click_access,metadata->>trust_level.eq.low)");
    } else if (["contact_revealed", "one_click_access", "lead_opened", "question_responded", "lead_received"].includes(emailType)) {
      query = query.eq("event_type", emailType);
    } else {
      query = query.eq("email_type", emailType);
    }
