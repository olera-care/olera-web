...
import {
  getCTAVariantWeights,
  saveCTAVariantWeights,
} from "@/lib/analytics/cta-variant-weights";

/**
 * GET /api/admin/analytics/cta-variant-weights
 *
 * Returns the current cta_variant weights + version. Behind admin
 * auth even though /api/variant-weights/cta is public — this one
...
export async function GET() {
  ...
  const record = await getCTAVariantWeights();
  return NextResponse.json(record);
}

/**
 * POST /api/admin/analytics/cta-variant-weights
 *
 * Body: { weights: Record<CTAVariant, number> } where each value is
 * a non-negative integer and the values sum to exactly 100. The valid
 * variant keys come from CTA_VARIANTS in lib/analytics/cta-variant.ts.
 *
 ...
 * assignCTAVariantWeighted for the hash-namespacing detail.
 *
...
export async function POST(request: NextRequest) {
  ...
  const result = await saveCTAVariantWeights(weights as Record<string, unknown>, user.id);
  if (!result.ok) {
