...
type WindowResult = {
  ...
  banner_leaderboard: Array<{ banner: string; impressions: number; clicks: number }>;
...
async function fetchWindow(
  ...
  // stage (repeat visits by one provider don't inflate the row).
  const bannerImpr = new Map<string, Set<string>>();
  ...
  for (const r of (bannerRes.data ?? []) as Array<{
    ...
    if (!r.provider_id) continue;
    const banner = typeof r.metadata?.banner === "string" ? r.metadata.banner : null;
    if (!banner) continue; // pre-instrumentation rows had no banner tag — skip
    const target = r.event_type === "provider_picker_clicked" ? bannerClick : bannerImpr;
    let set = target.get(banner);
    if (!set) { set = new Set<string>(); target.set(banner, set); }
    set.add(r.provider_id);
  }
  const bannerLeaderboard = [...new Set([...bannerImpr.keys(), ...bannerClick.keys()])]
    .map((banner) => {
      const impressions = bannerImpr.get(banner)?.size ?? 0;
      const clicks = bannerClick.get(banner)?.size ?? 0;
      return { banner, impressions, clicks };
    })
