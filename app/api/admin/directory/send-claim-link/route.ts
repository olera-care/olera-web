...
async function getPdfAttachment(): Promise<typeof cachedPdfAttachment> {
  ...
  try {
    ...
    if (!response.ok) {
      console.warn(`[directory/send-claim-link] Failed to fetch PDF: ${response.status}`);
      return null;
    }
    ...
    console.log("[directory/send-claim-link] PDF attachment loaded and cached");
    return cachedPdfAttachment;
  } catch (err) {
    console.warn("[directory/send-claim-link] Failed to fetch PDF attachment:", err);
    return null;
  }
}
 ...
 * Includes the "Olera for Providers" PDF one-pager attachment.
 *
 * Request body:
