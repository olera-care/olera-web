...
// Staff screening - 3 defaults for unclaimed providers
const STAFF_SCREENING_UNCLAIMED = [
  "Background checks",
  "Drug screening",
  "Reference checks",
];
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-6 lg:px-8 pt-6">
        ...
        <div className="md:flex md:gap-10 md:items-start mt-6">
          ...
          <div className="flex-1 min-w-0">
              ...
              <div className="space-y-6">
                ...
                <div className="flex gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-gray-500 text-sm">Rated {MOCK_PROVIDER.rating} stars based on {MOCK_PROVIDER.reviewCount} reviews on Google.</p>
                  </div>
                </div>
                ...
                <div className="flex gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-gray-500 text-sm">Provides {MOCK_PROVIDER.category.toLowerCase()} services for seniors in {MOCK_PROVIDER.city}, {MOCK_PROVIDER.state}.</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            {/* Reviews Section — matches production design */}
            <div id="reviews" className="scroll-mt-14 py-8 border-t border-gray-200">
              ...
              {/* Rating summary + See on Google link */}
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  <span className="text-3xl font-bold text-gray-900">{MOCK_PROVIDER.rating}</span>
                  <div className="flex">
                    {[...Array(5)].map((_, i) => (
                      <svg key={i} className={`w-5 h-5 ${i < Math.round(MOCK_PROVIDER.rating) ? "text-amber-400" : "text-gray-200"}`} fill="currentColor" viewBox="0 0 20 20">
                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                      </svg>
                    ))}
                  </div>
                  <span className="text-base text-gray-500">{MOCK_PROVIDER.reviewCount} reviews</span>
                </div>
                <a href="#" className="flex items-center gap-1.5 text-sm font-medium text-primary-600 hover:text-primary-700">
                  See all on Google
            ...
            <div id="screening" className="scroll-mt-14 py-8 border-t border-gray-200">
              ...
              <div className="grid grid-cols-2 gap-y-5 gap-x-8">
                {STAFF_SCREENING_UNCLAIMED.map((item) => (
            ...
            <div id="payment" className="py-8 scroll-mt-20 border-t border-gray-200">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {["Private Pay"].map((payment) => (
                  <div key={payment} className="flex items-center justify-between py-3">
                    <div className="flex items-center gap-3">
            ...
            <div className="pt-10 pb-8 border-t border-gray-200">
...
// =============================================================================
// Review Card Component (with Show more)
// =============================================================================
function ReviewCard({ review }: { review: { id: number; author: string; rating: number; date: string; text: string } }) {
  const [expanded, setExpanded] = useState(false);
  const maxLength = 150;
  const needsTruncation = review.text.length > maxLength;
  const displayText = expanded || !needsTruncation ? review.text : review.text.slice(0, maxLength) + "...";
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-primary-100 flex items-center justify-center">
            <span className="text-sm font-semibold text-primary-700">{review.author.charAt(0)}</span>
          </div>
          <div>
        ...
        <div className="flex items-center gap-1 text-sm text-primary-600">
          <span className="font-medium">{review.rating}.0 / 5</span>
      ...
      <div className="flex-1">
        ...
        {needsTruncation && (
          <button
            ...
            className="text-sm font-medium text-gray-900 underline mt-1 hover:text-gray-700"
          >
