    ...
    <main className="min-h-screen bg-[#FAFAF8]">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 pt-6 pb-32 lg:pb-12">
        ...
        <div className="lg:grid lg:grid-cols-[1fr_320px] lg:gap-8 lg:items-start">
              ...
              <div className="py-8 px-6 sm:px-8 border-t border-gray-200">
                ...
                {isVerified && (
                  <details className="mb-6 group">
                    <summary className="flex items-center gap-3 p-4 bg-emerald-50 rounded-xl border border-emerald-100 cursor-pointer list-none hover:bg-emerald-100/70 transition-colors">
                      ...
                      <svg className="w-5 h-5 text-emerald-400 group-open:rotate-180 transition-transform duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                      </svg>
                    </summary>
                    <div className="mt-2 ml-[52px] space-y-1.5">
                      {meta.drivers_license_expiration && (
                        <p className="text-xs text-gray-500">
                          <span className="font-medium text-gray-600">Driver&apos;s license</span> — expires {new Date(meta.drivers_license_expiration + "T00:00:00").toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}
                        </p>
                      )}
                      {meta.car_insurance_expiration && (
                        <p className="text-xs text-gray-500">
                          <span className="font-medium text-gray-600">Car insurance</span> — expires {new Date(meta.car_insurance_expiration + "T00:00:00").toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}
                        </p>
                      )}
                      {!meta.drivers_license_expiration && !meta.car_insurance_expiration && (
                        <p className="text-xs text-gray-400">Documents verified — expiration dates not listed.</p>
                      )}
                    </div>
                  </details>
                )}
          ...
          {/* ─── RIGHT COLUMN: CTA + About + References (Desktop Only) ─── */}
          <div className="hidden lg:block">
            <div className="sticky top-6 space-y-5">
              ...
              {meta.intended_professional_school && (
                <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
                  <h3 className="text-sm font-semibold text-gray-900 mb-2">Career Path</h3>
                  <p className="text-sm text-gray-600">
                    Pursuing {INTENDED_SCHOOL_LABELS[meta.intended_professional_school]}
                    {meta.graduation_year && ` · Graduating ${meta.graduation_year}`}
                  </p>
                  <p className="text-xs text-gray-400 mt-2">
                    Caregiving experience builds clinical skills and strengthens {INTENDED_SCHOOL_LABELS[meta.intended_professional_school]?.toLowerCase().includes("school") ? "" : "professional school "}applications.
                  </p>
                </div>
              )}
                    ...
                    {references!.slice(0, 3).map((ref) => (
                      <div key={ref.id} className="pb-3 border-b border-gray-50 last:border-0 last:pb-0">
                        <div className="flex items-baseline gap-2">
                          <p className="text-xs font-medium text-gray-900">{ref.referee_name}</p>
                          <span className="text-[10px] text-gray-400 capitalize">{ref.relationship}</span>
                        </div>
                        {ref.recommendation && (
                          <p className="mt-1 text-xs text-gray-500 leading-relaxed italic line-clamp-3">&ldquo;{ref.recommendation}&rdquo;</p>
                        )}
                      </div>
                    ))}
                  </div>
              ...
              {(reviews?.length ?? 0) > 0 && (
                <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
                  <h3 className="text-sm font-semibold text-gray-900 mb-2">Reviews</h3>
                  {(() => {
                    ...
                    return (
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex gap-0.5">
                          {[1, 2, 3, 4, 5].map((star) => (
                            <svg key={star} className={`w-4 h-4 ${star <= Math.round(avg) ? "text-amber-400" : "text-gray-200"}`} fill="currentColor" viewBox="0 0 20 20">
