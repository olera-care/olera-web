...
function getActionRedirectUrl(
  ...
  if (action) {
    switch (action) {
      ...
      case "verify":
        // Onboarding verification nudge → settings with verification modal auto-open.
        return "/account/settings?verify=true";
      case "notifications":
        // Onboarding notification setup nudge → settings on the Notifications tab.
        return "/account/settings?tab=notifications";
      case "profile":
        // Onboarding profile preview → the provider's public page.
...
export default function ProviderOnboardPage() {
  ...
  const actionParam = searchParams.get("action") as NotificationType | "campaign" | "claim" | "signup" | "manage" | "settings" | "market" | "ads" | "leads" | "matches" | "verify" | "notifications" | "profile" | null;
                ...
                try {
                  ...
                  if (user && account) {
                    ...
                    if (ownedProfile) {
                      ...
                      if (actionParam === "manage" || actionParam === "settings" || actionParam === "market" || actionParam === "ads" || actionParam === "leads" || actionParam === "verify" || actionParam === "notifications" || actionParam === "profile") {
