...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!data) return { title: "Candidate Not Found | Hire Staff" };
...
export default async function ProviderStudentProfilePage({ params }: PageProps) {
  const { slug } = await params;
    ...
    <main className="min-h-screen bg-[#FAFAF8]">
      ...
      <div className="bg-white border-b border-gray-100">
        <div className="max-w-5xl mx-auto px-4 sm:px-6">
          {/* Breadcrumb */}
          <nav className="flex items-center gap-2 text-sm text-gray-400 pt-6 pb-4">
            <Link href="/provider/medjobs/candidates" className="hover:text-primary-600 transition-colors">
              ← Back to Candidates
            </Link>
          </nav>
                ...
                <div className="rounded-2xl overflow-hidden bg-gray-100 shadow-sm">
                  {videoAvailable && youtubeId ? (
                    ...
                    <a
                      ...
                      className="flex items-center justify-center gap-4 py-20 sm:py-24 hover:bg-gray-50 transition-colors"
                    >
                      <div className="w-20 h-20 rounded-full bg-primary-100 flex items-center justify-center shadow-lg">
                        <svg className="w-10 h-10 text-primary-600 ml-1" fill="currentColor" viewBox="0 0 24 24">
                          <path d="M8 5v14l11-7z" />
                        </svg>
                      </div>
                      <div>
                        <p className="text-lg font-semibold text-gray-900">Watch Intro Video</p>
                ...
                <div className="flex gap-3">
                  {resumeUrl && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-gray-50 hover:bg-gray-100 border border-gray-200 rounded-xl text-sm font-medium text-gray-700 transition-colors"
                    >
      ...
      {/* ═══════════════════════════════════════════════════════════════════
          CONTENT SECTIONS
          ═══════════════════════════════════════════════════════════════════ */}
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 lg:py-12 space-y-8 lg:space-y-12 pb-32 sm:pb-12">

        {/* ─── SECTION 1: About ─── */}
        {(meta.why_caregiving || profile.description || meta.intended_professional_school) && (
          <section>
        ...
        <section>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 sm:p-8">
            ...
            {/* Schedule Grid (collapsed by default in a details element) */}
        ...
        {(meta.acknowledgments_completed || meta.ncns_pledge || meta.school_balance_pledge || meta.advance_notice_pledge || meta.prn_willing) && (
          <section>
            <h2 className="text-xl font-display font-bold text-gray-900 mb-4">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 sm:p-8">
              ...
              <div className="space-y-6">
                {meta.scenario_responses.map((sr, i) => (
                  <div key={i} className="border-l-3 border-primary-200 pl-5">
        ...
        {/* ─── Social Links (if any beyond LinkedIn) ─── */}
        {(meta.instagram_url || meta.facebook_url || meta.tiktok_url) && (
          <section>
            <h2 className="text-xl font-display font-bold text-gray-900 mb-4">
              Social
            </h2>
            <div className="flex gap-3">
              {meta.instagram_url && (
                <a href={meta.instagram_url} target="_blank" rel="noopener noreferrer" className="w-12 h-12 rounded-xl bg-white border border-gray-200 hover:border-pink-300 flex items-center justify-center transition-colors" title="Instagram">
                  <svg className="w-5 h-5 text-gray-600 hover:text-[#E4405F]" fill="currentColor" viewBox="0 0 24 24">
              ...
              {meta.facebook_url && (
                <a href={meta.facebook_url} target="_blank" rel="noopener noreferrer" className="w-12 h-12 rounded-xl bg-white border border-gray-200 hover:border-blue-300 flex items-center justify-center transition-colors" title="Facebook">
              ...
              {meta.tiktok_url && (
                <a href={meta.tiktok_url} target="_blank" rel="noopener noreferrer" className="w-12 h-12 rounded-xl bg-white border border-gray-200 hover:border-gray-400 flex items-center justify-center transition-colors" title="TikTok">
