...
export default function ProviderProfilePage() {
  ...
  // Pre-fill form from provider profile
  useEffect(() => {
    if (!providerProfile) return;

    setImageUrl(providerProfile.image_url || null);
    setForm({
      ...
      care_types: providerProfile.care_types || [],
    });

    const meta = providerProfile.metadata || {};

    if (providerProfile.type === "organization") {
    ...
    } else if (providerProfile.type === "caregiver") {
  ...
  // providerProfile is guaranteed by the provider layout guard
  ...
  const buildMetadata = (): Record<string, unknown> => {
    ...
    if (providerProfile.type === "caregiver") {
      const m: Record<string, unknown> = {};
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      formData.append("profileId", providerProfile.id);
