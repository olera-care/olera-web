    ...
    >
      ...
      <div className="flex-1 p-4 flex flex-col">
        ...
        <div className="flex items-start justify-between gap-2">
          <h3 className="font-display font-semibold text-text-md text-gray-900 group-hover:text-primary-700 transition-colors line-clamp-2 flex-1 leading-snug">
            {provider.name}
          </h3>
          {provider.rating > 0 && (
            <div className="flex items-center gap-1 flex-shrink-0 mt-0.5">
              ...
              <span className="text-text-sm font-semibold text-gray-900">{provider.rating.toFixed(1)}</span>
            </div>
          )}
        </div>

        {/* Category + Location */}
        <p className="text-text-sm text-gray-500 mt-1 line-clamp-1">
        ...
        {displayedHighlights.length > 0 && (
          <div className="flex flex-wrap gap-x-3 gap-y-1 mt-3">
            {displayedHighlights.map((h) => (
              <span key={h} className="flex items-center gap-1.5 text-text-xs text-gray-600">
        ...
        {/* Price */}
        <p className="text-text-sm font-bold text-gray-900 mt-3">{provider.priceRange}</p>
      </div>
    </Link>
  );
}
