...
export default function AmenitiesSection({ categories, highlightPhoto, headerAction }: AmenitiesSectionProps) {
  ...
  if (hasDetailedItems) {
    ...
    // Default to 3 columns for 3+ categories, 2 for fewer
    const colCount = visible.length >= 3 ? 3 : 2;
    ...
    return (
      <div id="amenities" className="py-8 scroll-mt-20 border-t border-gray-200">
        ...
        <div className={`grid grid-cols-1 gap-4 ${colCount === 3 ? "sm:grid-cols-3" : "sm:grid-cols-2"}`}>
          {visible.map((cat) => (
