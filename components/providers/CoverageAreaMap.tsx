...
/** Olera house-pin marker SVG — teal bubble with white house icon and pointer tail */
...
/** Green "covered" pin with checkmark */
...
/** Expand polygon outward by a rough padding (in degrees) */
...
const CoverageAreaMap = forwardRef<CoverageAreaMapHandle, CoverageAreaMapProps>(
  function CoverageAreaMap({ center, areas, providerName }, ref) {
    ...
    const focusArea = useCallback((lat: number, lng: number, name: string) => {
    ...
    const dropPin = useCallback((lat: number, lng: number, covered: boolean) => {
      ...
      // Show coverage polygon so user can see the boundary
      showCoveragePolygon();
    ...
    useEffect(() => {
      ...
      const init = async () => {
        ...
        // Olera house-pin icon for all markers
