...
interface SaveButtonProps {
  ...
  variant?: "default" | "icon" | "pill" | "pill-quiet";
}
...
export default function SaveButton({ provider, variant = "default" }: SaveButtonProps) {
  ...
  const tooltip = showTooltip && (
    <div
      ref={tooltipRef}
      className="absolute z-50 top-full mt-2 right-0 w-56 sm:w-64 bg-gray-900 text-white rounded-xl shadow-lg p-3 animate-in fade-in slide-in-from-top-1 duration-150"
    >
      ...
      <div className="relative">
        <button
          ...
          className={`flex items-center gap-1.5 text-sm font-medium transition-colors ${
            saved ? "text-primary-600" : "text-gray-700 hover:text-gray-900"
          }`}
        >
          <svg
            className="w-4 h-4"
