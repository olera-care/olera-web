...
interface MoreBottomSheetProps {
  ...
  /** New leads count */
  leadsCount: number;
  /** Active provider profile id — scopes the NavBadge "new since last look" mark */
...
interface MenuItem {
  ...
  /** Pop the badge when the count rises since this browser last saw it */
          ...
          {MENU_ITEMS.map((item) => {
            ...
            return (
              ...
              >
                ...
                <NavBadge
                  count={badge}
                  seenKey={item.popOnIncrease && props.profileId ? `${item.key}:${props.profileId}` : null}
                  className="min-w-[24px] h-6 flex items-center justify-center px-2 text-xs font-bold text-white bg-red-500 rounded-full"
                  maxDisplay={99}
                />
              </Link>
            );
