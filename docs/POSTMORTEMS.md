...
### 2026-04-02: Pipeline batch CSV parsing crash — quoted fields with commas

**Symptom**: `pipeline-batch.js` crashed with `FATAL: CsvError: Invalid Record Length: columns length is 18, got 35 on line 57` on city 17/116 (Frederickson, WA). The entire batch pipeline halted.

**Root Cause**: Google Places API returns a `types` field with comma-separated values (e.g., `"health,point_of_interest,service,establishment"`). The discovery script wraps these in CSV quotes, but quoting is inconsistent across rows. `csv-parse` with `{ columns: true }` enforces strict column counts and threw on the first row where quotes didn't contain the commas.

**Fix**: Added `relax_column_count: true, relax_quotes: true` to both `csvParse.parse()` calls in `pipeline-batch.js` (lines 530 and 869).

**Time to Resolution**: ~10 minutes.

**Prevention**: Any script parsing discovery CSVs should use relaxed parsing. The discovery script should also ensure consistent quoting of the `types` field.

**Lesson**: When parsing CSVs generated by another script in the pipeline, always use defensive parsing options. A strict parser turns a data quirk into a pipeline-halting crash.

---

### 2026-04-02: Pipeline batch parser rejected plain CSV — expected "Machine-Readable" header

**Symptom**: `pipeline-batch.js --batch batch.csv` returned `ERROR: No cities found` despite 116 valid `city,state` lines.

**Root Cause**: `parseBatchMd()` falls back to `content.slice(content.indexOf('Machine-Readable'))`. When `indexOf` returns `-1`, `slice(-1)` returns only the last character — 0 parsed cities. The comment "same parser works for simple CSVs" (line 1260) was wrong.

**Fix**: Converted `.csv` to `.md` with a "Machine-Readable List" header. Proper fix: add a fallback that parses all non-empty lines as `city,state` when no markdown markers found.

**Time to Resolution**: ~5 minutes.

**Prevention**: The `parseBatchMd()` function should handle plain CSV files. The misleading comment on line 1260 should be fixed.

**Lesson**: When code claims to support multiple formats via comments, verify the actual code path — a comment claiming compatibility doesn't make it so.
