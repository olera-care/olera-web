...
| `Olera_Pre_CRP_RD_Commercialization_and_Execution_Plan.pdf` / `.docx` | **The pre-CRP plan**, ten sections: the eldercare ecosystem and care-establishment pathway (Fig. 1), the five-year sequence, provider commercialization, CareNavigator's current state and its CRP-entry definition, institutional-buyer development, the investor thesis, the January scorecard organized by the eight risks it addresses, and the eighteen-week plan to submission | build in `passes/pre-crp-memo-src/`, 2026-09-02 | live; governs the September to January window |
