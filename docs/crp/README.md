...
| `Olera_Pre_CRP_RD_Commercialization_and_Execution_Plan.pdf` / `.docx` | **The pre-CRP plan**, seven sections: purpose, the strategic model (Fig. 1), the R&D and commercialization roadmap, the three Phase IIB and pre-CRP objectives (provider commercialization, CareNavigator first-generation development, institutional-buyer discovery), private investors, the January scorecard of six risks, and the eighteen-week plan to 1 January | Logan's iteration 4, 2026-09-02; house-style build in `passes/pre-crp-v4-src/` | live; governs the September to January window |
