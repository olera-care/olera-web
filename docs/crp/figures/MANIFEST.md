...
| Fig A | `fig-approach-overview.svg` | Aims 1 and 2 verify, validate, and build adoption in the same local markets; Aim 3 tests whether provider revenue sustains Olera after the award and produces the commercial evidence investors have asked for. | Opener of the Approach section, after the Rationale paragraph | The entire Approach in one view, reading left to right: broad national traction → local market depth → real commercial test → post-award sustainability and replication. Task tags 1.1–3.3 link stages to the detailed sections. No market counts. | **current** — adopted 2026-08-19 (Logan); lettering finalized in the consistency sweep |
