...
|---|---|---|---|---|---|
| Fig 1 | `fig-1.png` | Two constraints produce one crisis. Aid that already exists goes unclaimed, and the workforce to deliver care does not exist. CareNavigator addresses both. | Significance, after "The unmet need" | The two-sided problem framing that justifies a two-sided product | **needs-revision**: artwork says "~80% annual turnover"; verified figure is 75.0% (Activated Insights 2024) |
...
| **Proposed: Approach overview** | `fig-approach-overview.svg` | (caption to be set) Two parallel streams — Aim 1 validates the free family product and grows demand; Aim 2 validates the three provider products and grows the provider/student base — feed a marketplace concentrated in selected counties; Aim 3 prices what proved its value and tests whether provider revenue sustains the platform. | proposed opener of the Approach section, before "Overall design and timetable" | The entire Approach in one view — the figure the reviewer reads before any protocol detail | **proposed** — authored 2026-08-19, awaiting Logan/team adoption; would complement or replace Table 1 as the orientation device |
| Table 1 | `table-1.png` | Timetable and go/no-go gates. Aims 1 and 2 run in parallel, and each offering graduates into Aim 3 on its own gate. | Approach, "Overall design and timetable" | Verify → Validate/adopt → Prove structure by year and aim | **needs-revision**: says "All four modules pass in test" — taxonomy is now three products; rebuild as editable SVG or real table during Aim 2 pass |
