...
| Fig 2 | `fig-product.svg` | The two sides of the marketplace and what CareNavigator does for each. | Significance, after "The product and the business model" (signposted in that paragraph) | What the product actually is, so the reader carries the right mental model forward: a family works through the CareNavigator interface, which understands the need, identifies what fits, and helps the family act, while care providers who need clients and staff use the Provider Growth Suite to reach families, convert inquiries, and find staff. Revised 2026-08-19 (Logan) to add the provider side as a mirror of the family side: step subtext removed and icons moved beside their headings to make room, a pale band behind both halves showing one marketplace rather than two products. Recreates the Phase IIB user-journey figure (family, interface, what the system does) with three changes required by current prose and product truth: the three panels are renamed to the prose's three moves, "Multi-Agent Network" becomes "AI-assisted navigation" (the agentic layer is in a separate codebase and is never described as deployed), and the third panel carries an explicit note that preparation and follow-through are completed under this award. Phone screen shows a real intake question from the live seven-question finder. | **current** — authored 2026-08-19 |
| Fig 3 | `fig-2.png` | One loop, two sides. Consented family demand is the reason a provider signs in. Whether it is also what keeps them paying is the hypothesis Aim 3 tests. | End of Innovation | The flywheel: benefits directory → aid secured → family shares profile → providers arrive → Growth Suite revenue; answers "three companies at once" | current |
| Fig 4 | `fig-3.png` | Demand arrives on its own. Organic search alone brings more than 15,000 families a month at near-zero acquisition cost, in nearly every county in the country. | Preliminary Work | Measured organic traction (May 12,312 / June 13,913 / July 2026 15,530; 3,142 counties; 72,000+ records; 725 providers; 2,400 leads/mo) — the acquisition-cost advantage | current — numbers should be refreshed near submission |
| Fig 5 | `fig-4.png` | What the CareNavigator does for one family. Seven questions match one Houston household to twenty programs worth an estimated $955 a month. The platform prepares each application and the family submits it. | Preliminary Work | Concrete worked example of the family-side value | current |
| Fig 6 | `fig-5.png` | The economic hypothesis the award tests. Provider revenue reaches break-even by the end of the award, and private capital accelerates a model already shown to work. Trajectories are illustrative. | End of Aim 3 | Award-end break-even + two post-award paths; carries "~300 paying accounts · ~$720K ARR · <12-month payback · 3:1 LTV:CAC" | **needs-revision**: end-state numbers are provisional pending Aim 3 redesign and RS↔CP reconciliation |
