...
| Fig 2 | `fig-product.svg` | What CareNavigator does: understand the need, identify the aid and services, help the family act. | Significance, after "The product and the business model" (signposted in that paragraph) | What the product actually is, so the reader carries the right mental model forward: a family works through the CareNavigator interface, which understands the need, identifies the aid, services, and providers that fit, and helps the family act. Recreates the Phase IIB user-journey figure (family, interface, what the system does) with three changes required by current prose and product truth: the three panels are renamed to the prose's three moves, "Multi-Agent Network" becomes "AI-assisted navigation" (the agentic layer is in a separate codebase and is never described as deployed), and the third panel carries an explicit note that preparation and follow-through are completed under this award. Phone screen shows a real intake question from the live seven-question finder. | **current** — authored 2026-08-19 |
