# -*- coding: utf-8 -*-
"""Build the house-style .docx for the competitive environment section."""
import re, html as H
...
SRC = open('ce_word.html', encoding='utf-8').read()
...
def add_runs(par, frag, size=None, color=None, base_bold=False, bold_color=None):
    bold, ital, sup = base_bold, False, False
...
def cell_margins(tbl, lr_twips):
    """Word's default 0.08in side margin on every cell is wider than the rating
    columns can spare, and a fixed-layout table absorbs it by breaking the header
    words mid-syllable. Reduce it rather than reduce the type."""
    m = OxmlElement('w:tblCellMar')
    for e, v in (('top', '0'), ('left', str(lr_twips)),
                 ('bottom', '0'), ('right', str(lr_twips))):
        x = OxmlElement('w:' + e); x.set(qn('w:w'), v); x.set(qn('w:type'), 'dxa')
        m.append(x)
    tbl._tbl.tblPr.append(m)
...
def build_table(thtml, keep=False, plain=False):
    ...
    for ri, row in enumerate(rows):
        ...
        shaded = ri < len(tr_attrs) and 'class="us"' in tr_attrs[ri]
        for ci, (tag, attrs, content) in enumerate(row):
            cell = tbl.cell(ri, ci); cell.text = ''
            par = cell.paragraphs[0]
            centred = 'class="m"' in attrs
            par.alignment = (WD_ALIGN_PARAGRAPH.CENTER if centred
...
BLOCK = re.compile(r'(<h1[^>]*>.*?</h1>|<div class="figfloat".*?</div>|'
                   r'<div class="fig">.*?</div>|<ol class="risks">.*?</ol>|'
...
doc.save('Olera_CRP_CompetitiveEnvironment.docx')
print('saved docx')
