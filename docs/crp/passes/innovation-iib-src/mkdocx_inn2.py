# -*- coding: utf-8 -*-
"""Build the house-style .docx for the Innovation (Phase IIB) section."""
import re, html as H
...
SRC = open('inn2_word.html', encoding='utf-8').read()
...
def add_figure(img, w, cap, floated=False):
    ...
    if not floated:
        ...
        return
    # Writer still insets content a little inside a frame even with zero cell
    # margins, which shaves the right edge off a full-width image. Give the
    # frame a hair more width than the picture.
    fw = w + 0.10
    t = doc.add_table(rows=1, cols=1); t.autofit = False
    t.columns[0].width = Inches(fw); t.rows[0].cells[0].width = Inches(fw)
    no_borders(t); zero_cell_margins(t); fixed_width(t, fw); float_table(t, fw)
    c = t.cell(0, 0); c.text = ''
...
doc.save('Olera_CRP_Innovation_PhaseIIB.docx')
print('saved docx')
