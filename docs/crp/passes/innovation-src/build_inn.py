...
Source: Logan's Innovation draft, 2026-08-30. Prose is the author's. The figures
are redrawn in the house vector system from the draft's illustrated versions:
same content, same reading order, no raster, so they hold at any print size and
match the rest of the application.
"""
import os, re
import figs_inn as F
...
def figblock(svg, num, cap):
    """A figure and its caption are one indivisible block."""
    if WORD:
...
FIG1 = figblock(F.fig1(), 1,
    "Real-world events along the care-establishment pathway become persistent, computable state.")
FIG2 = figblock(F.fig2(), 2,
    "From information to execution. CareNavigator agents work real-world channels, persist "
    "across time, and verify that each step actually completed.")
FIG3 = figblock(F.fig3(), 3,
    "From redistributing the same workforce to creating new capacity, directed to where the "
    "pathway is failing.")
...
journey can be computed, tracked, and learned from. As the family moves through the eldercare
ecosystem, CareNavigator records events, tasks, responses, and outcomes, updating the case state at
each step (Figure 1). This longitudinal record is the foundation the other two innovations stand on:
AI execution that can act on the right next step, and capacity intelligence that detects when
workforce shortages are blocking care.</p>
...
longitudinal case state, our agents plan the next action, use the appropriate tools, interact across
...
it to the places where families are being blocked (Figure 3). We launch with pre-health students, a
large and well-aligned population seeking patient-care experience; in our pilot this approach already
attracted more than 900 applicants. We recruit through universities, pre-health organizations, career
centers, and digital outreach. The same infrastructure will expand to other new labor pools, such as
students in health-related fields, career changers, and trained workers not currently in direct
care.</p>

{FIG3}

<p class="sec"><b>The end product: the CareNavigator experience.</b> To families, these innovations
appear as one system. Through the web and mobile experience, families see what is needed, what
CareNavigator is doing, what is pending, what requires their approval, and whether care has been
established. The same platform carries very different real-world cases across the eldercare ecosystem
to the same endpoint (Figure 4).</p>
