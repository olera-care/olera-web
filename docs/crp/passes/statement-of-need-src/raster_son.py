import re, subprocess, pymupdf, figs_son as F
for n, svg in {1: F.fig1(), 2: F.fig2(), 3: F.fig3()}.items():
    ...
    subprocess.run(['/opt/pw-browsers/chromium','--headless','--disable-gpu','--no-sandbox',
                    '--no-pdf-header-footer', f'--print-to-pdf=png/fig{n}.pdf',
                    f'png/fig{n}.html'], capture_output=True)
    ...
    print(f'fig{n}: {w_in:.2f} x {h_in:.2f}in -> {pix.width} x {pix.height}px')
    d.close()
