...
- `make_client` (`handleMakeClient` in `route.ts`) — writes `business_profiles.metadata.interview_terms_accepted_at`, transitions status to `active_partner`, derives Stage to `converted`, unlocks catchment Partner Prospects via `partner-prospect-gate.ts`
...
All four Log modules gate this correctly:
- **Call modal:** filters `convert_to_partner` vs `convert_to_client` by `rowKind`
- **Pre-flight modal:** engagement options offered are provider-appropriate
- **Reply modal:** `committed` (stakeholder) vs `became_client` (provider), gated by `rowKind`
- **Meeting modal:** `done_partner` (stakeholder) vs `done_client` (provider), gated by `rowKind`
