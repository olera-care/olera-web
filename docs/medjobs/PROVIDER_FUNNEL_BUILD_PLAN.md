...
**Update — student accept/decline surface wired (2026-06-15):** `/portal/medjobs/interviews` now loads placements and passes them to `InterviewCalendar`; the **student view** shows "Accept offer / Decline" on a confirmed interview that has an open offer, and a confirmed banner once accepted (accept → confirmed; Stripe still stubbed). Provider offers are **idempotent** (re-offering returns the existing placement, no double-offer). **Loop 3 is now click-through end-to-end without payments.** Still remaining: Stripe (authorize-at-offer/capture-at-confirm + refund guard), contact unlock at confirm, post-interview emails/reminders, and the provider view loading placements (cosmetic — idempotency covers correctness).

**Update — cold-send `{welcome_url}` confirmed + made multi-use (2026-06-15):** the cold eligibility link was *already* wired — `smartlead-bridge.rowToLeads` sets a per-provider magic link as the `welcome_url` custom field and `finalizeTokens` maps `{welcome_url}` → the `{{welcome_url}}` Smartlead merge tag. The only real gap was that the provider magic link was **single-use** (JTI burned on first visit), which breaks across a 3-email cadence that reuses one baked link. Fixed: `app/medjobs/m/[token]/route.ts` now treats the provider link as **multi-use** (audits once, no longer blocks re-visits) — mirroring the partner-portal token rationale (still email-bound + 30-day TTL). **To activate in Smartlead:** ensure `MEDJOBS_MAGIC_LINK_SECRET` is set, run the **sequence refresh** (`/admin/medjobs/smartlead-refresh`) to push the new copy, and the **leads refresh** (`/admin/medjobs/smartlead-refresh-leads`) to regenerate a fresh `welcome_url` on existing leads.
