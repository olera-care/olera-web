...
professors, each activating on its own schedule. Adding one is a button; each one keeps its own contacts,
its own history and its own next check.
...
Records inside the list channels can be **declined** in the same way, for the organization that says no or
the professor who is not interested. A declined record stops appearing as unfinished work.
...
Work top down. Universities with a red dot sort first, so when the dots run out, you are done for the day.

Click a university to open it. You will see its five channels, each collapsed to one line: the status, how
far along it is, and the date it is next due. Click a channel to open its workflow. Click a date to jump
straight to the task that explains what to do.

---

## Working a channel

Inside a channel you will find the same six things every time, in the same order.

**Live win** is the checklist. Tick what has actually happened. Each tick records the date automatically,
so you never type one.

**Contacts** default to the university advisor, because they are usually the right person. Where the real
contact is someone else, a career services officer for the job board, an events office for a fair, add
them.

**Suggested email and call script** are there where the step involves asking someone for something. They
are short on purpose. A partner who has to edit is a partner who does not send. Edit them freely and copy
them out; the workspace does not send mail.

**Next check** is the one open follow-up. Click it and you land on the task.

**History** is collapsed until you want it, with the count on the toggle. Add a note whenever something
happens that the checkboxes do not capture.

**Mark not available here** sits at the bottom, and asks for a reason.

### The three list channels

**ST5, student organizations.** Add each organization by name. Each one is live as soon as its leadership
agrees to any one meaningful way of reaching members: sharing the flyer in a group chat, emailing it out,
or arranging a presentation. One is enough. Each live organization gets its own monthly reconnect.

**ST6, campus events.** Add both kinds: webinars we host and university events we join. An event is live
when five things are true, a leader identified, host approval obtained, a date scheduled, a presenter
assigned, and collateral ready. The same five work for a webinar and for a career fair. This channel also
carries a **semester review**, which arrives on its own so that nothing sits unlooked-at for a term.

**ST7, professors.** This one is gated. **You cannot add a professor until approval is recorded**, because
we do not prospect faculty until a university has told us we may. Record who can give that permission, then
record it when it arrives. The list unlocks then, and not before. Once a professor agrees, either to pass
the opportunity on or to let us join a class, they are live and come back to you seasonally.
...
Open a task and it tells you what it is, why it matters, who to contact, the steps, and what done means.
Where the task is a message, the draft is there to copy.

Two kinds of button appear at the bottom, and the difference matters.
...
**Yes, they agreed** appears on the one task that asks a question rather than setting a chore. Answering it
ticks the box back in the university record. The task and the checklist are the same fact, so they cannot
end up disagreeing. **Not yet, ask again** pushes it out a week and leaves the box alone.
...
Every path goes both ways.

A next-check date in a university opens that task. A task names its university and channel as a link back.
A red dot anywhere leads to the item that caused it. You can start from either tab and never reach a dead
end.
