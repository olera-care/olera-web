# MedJobs 1.0 — Execution Playbook
...
|---|---|---|---|---|---|
| **S0** | Site setup & data | **Ces** | A university is named | Catchment audited, provider data clean, partner list built | `P1 §1` |
| **S1** | Prospect → Pre-Flight | **Graize** | Providers materialized | Required contact fields present | `P1` |
| **S2** | Cadence & call queue | **Graize** | Pre-flight clears | Reply, meeting, or cadence exhausted | `P2` |
| **S3** | Reply triage → booked meeting | **Graize** → **Chantel** | Any inbound signal | Meeting on the calendar | `P3` |
| **S4** | Meeting → Client | **Chantel** (Logan runs the meeting in Wks 1–4) | Meeting held | Terms accepted, provider onboarded | `P4` |
| **S5** | Supply: partners → candidates | **Ces** | Site setup done — **runs in parallel with S1–S4, never after** | Live candidates on the board | `P5`, `P6` |
...
2. **Do not book a conversion meeting into an empty site.** The site needs a **minimum viable board — 8 live
   candidates** — before S4 meetings are scheduled. If demand is running ahead of supply, Graize keeps
...
|---|---|---|---|
| 1 | **Ces** | Catchment audit — pull the provider list for Site #1, verify these are real non-medical home care agencies in the right cities, flag junk | An audited list with a defensible count |
| 1 | **Ces** | Fix provider data on the top 40: name, phone, email, address, website | 40 rows with required fields |
...
| 2 | **Graize** | Materialize the first 40 provider prospects; run `P1` pre-flight on all 40 | 40 rows past the launch gate or explicitly parked |
| 2 | **Ces** | Build the partner list for Site #1: pre-health advisors, pre-med/nursing/public-health org leaders, relevant department heads. Target 15–25 named humans with email + phone | Partner prospects created in the Partners tab |
...
| 3 | **Logan** | Recover Diana's flyer (**O-6**) or approve the current one as the Week-1 baseline | A flyer asset exists that Ces can send |
...
| Day | Owner | Task | Done when |
|---|---|---|---|
| 4 | **Graize** | Launch the provider cadence — **~25 sends/day maximum**, per `../EMAIL_LAUNCH_PLAN.md §4`. Resist the urge to go bigger; reputation damage is not recoverable inside this quarter | Campaign live; Day-0 emails delivered |
| 4 | **Ces** | Launch partner outreach in parallel. **Do not wait for the first Client** | Partner cadence live |
| 4–7 | **Graize** | Work the Calls tab to zero every day; log every outcome | Calls tab empty daily |
| 4–7 | **Chantel** | Reply triage within 1 business day; book meetings — **book, don't explain** (`P3`) | First meeting on the calendar |
| 5 | **Ces** | First partner meetings; ask for distribution, get the flyer moving | ≥1 partner agrees to distribute |
...
| 8 | **Ces** | Flyer physically circulating: org GroupMe/Slack/IG, advisor list, one physical posting | Distribution evidence logged on the partner row |
| 9 | **Ces** | First student applications land; shepherd them to complete (`P6`) | ≥3 applications started, ≥1 complete |
...
| **Daily** | Reply SLA: every inbound answered within 1 business day; meeting requests same day | Chantel (provider), Ces (partner/student) | continuous |
...
| **Wed** | Supply/demand balance check: live candidates per active client | Ces + Chantel | 15 min |
| **Thu** | Pipeline review: every row with a meeting or an open reply | Graize + Chantel | 20 min |
...
|---|---|---|---|
| **Data** | Wrong number, closed business, not actually home care, out of catchment | **Ces** | Fix the row, then check whether the whole batch shares the defect |
...
|---|---|---|---|
| **Ces → Graize** | Site data audited | Clean provider rows with required fields, catchment confirmed | Before prospecting starts |
| **Graize → Chantel** | Meeting booked, or a reply that needs a commercial answer | Row link, contact name, what they said, what they asked for | Same day |
| **Chantel → Ces** | New Client activated | Site, agency name, roles wanted, shift types, how many | Within 1 day — this is the demand signal supply needs |
| **Ces → Chantel** | Candidate goes live | Candidate profile, availability, site | Weekly batch is fine |
...
## 6. What we deliberately are not doing in 1.0
...
| Self-serve everything | The operator *is* the product in 1.0 | Never fully — this is a service |
