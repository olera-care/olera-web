# -*- coding: utf-8 -*-
"""Build the house-style .docx for the MedJobs walkthrough summary.
...
SRC = open('wt_word.html', encoding='utf-8').read()
...
while i < len(blocks):
    ...
    if b.startswith('<h2'):
        ...
        if i + 1 < len(blocks) and 'class="caption"' in blocks[i + 1]:
            ...
            # the stylesheet's 5pt bottom margin collapses into the next
            # heading's 11pt top margin; Word adds them, so this side is zeroed
            pc.paragraph_format.space_after = Pt(0)
            exact(pc, 10.44)
            add_runs(pc, cap, size=9)
        ...
        if 'wkh' in cls:
            ...
            add_runs(p, txt, bold_color=TEAL)
        elif 'lede' in cls:
            p.alignment = WD_ALIGN_PARAGRAPH.LEFT
...
doc.save('MedJobs_Operating_System_Walkthrough_Summary.docx')
print('saved docx')
