...
export const CRON_REGISTRY: CronJob[] = [
  ...
  {
    id: "building-payment-insurance",
    name: "Building: Payment & insurance",
    description: "Building Email 1. Asks providers to add accepted payment and insurance options, 5 days after the notification setup nudge. Skips providers who already have payment info or are above 80% completion.",
    recipientCohort: "Claimed organizations that completed onboarding through the notification nudge, with no accepted payments, and no prior attempt.",
    audience: "Providers",
    fn: "nudge",
    schedule: "0 * * * *",
    humanSchedule: "Hourly (Mon-Fri, 9am-5pm provider local)",
    path: "/api/cron/building-payment-insurance",
    emailTypes: ["building_payment_insurance"],
    successSignal: "Provider adds at least one accepted payment method.",
    relatedAdminPath: "/admin/provider-comms",
  },
  {
    id: "building-availability",
    name: "Building: Availability check",
    description: "Building Email 2. Asks providers whether they are currently accepting new clients via a one-click Yes/No in the email. Universal — every provider gets this. Availability is not currently a portal feature, so this is the only way to collect it.",
    recipientCohort: "Claimed organizations that completed onboarding, 5 days after payment email (or 10 days after notification nudge if payment was skipped).",
    audience: "Providers",
    fn: "nudge",
    schedule: "0 * * * *",
    humanSchedule: "Hourly (Mon-Fri, 9am-5pm provider local)",
    path: "/api/cron/building-availability",
    emailTypes: ["building_availability"],
    successSignal: "Provider clicks Yes or No — availability status recorded.",
    relatedAdminPath: "/admin/provider-comms",
  },
  {
    id: "building-facility-manager",
    name: "Building: Facility Manager",
    description: "Building Email 3. Asks providers to add a facility manager (name, photo, bio) to put a face behind their community. Skips providers at or above 80% completion or with facility manager already filled out.",
    recipientCohort: "Claimed organizations that received the availability email, below 80% completion, with no facility manager info and no prior attempt.",
    audience: "Providers",
    fn: "nudge",
    schedule: "0 * * * *",
    humanSchedule: "Hourly (Mon-Fri, 9am-5pm provider local)",
    path: "/api/cron/building-facility-manager",
    emailTypes: ["building_facility_manager"],
    successSignal: "Provider adds facility manager name and details.",
    relatedAdminPath: "/admin/provider-comms",
  },
  {
    id: "building-care-services",
    name: "Building: Care Services",
    description: "Building Email 4. Suggests 3 category-specific services and asks providers to confirm or edit. Dynamic content based on provider_category. If confirming pushes profile to 80%, provider graduates to growth stage.",
    recipientCohort: "Claimed organizations that received the availability email, below 80% completion, with empty care_types and a recognized provider_category.",
    audience: "Providers",
    fn: "nudge",
    schedule: "0 * * * *",
    humanSchedule: "Hourly (Mon-Fri, 9am-5pm provider local)",
    path: "/api/cron/building-care-services",
    emailTypes: ["building_care_services"],
    successSignal: "Provider confirms suggested services or edits services in portal.",
    relatedAdminPath: "/admin/provider-comms",
  },
  {
    id: "building-photos",
    name: "Building: Photos",
    description: "Building Email 5 (final). Asks facility-based providers (assisted living, memory care, nursing homes, independent living) to add photos of their community. Skips home care and home health. After this email is sent, the provider graduates to the growth stage.",
    recipientCohort: "Facility-based organizations with fewer than 3 photos, not admin-archived, 7 days after care services (or previous anchor if skipped).",
    audience: "Providers",
    fn: "nudge",
    schedule: "0 * * * *",
    humanSchedule: "Hourly (Mon-Fri, 9am-5pm provider local)",
    path: "/api/cron/building-photos",
    emailTypes: ["building_photos"],
    successSignal: "Provider uploads photos to their gallery.",
    relatedAdminPath: "/admin/provider-comms",
  },
  {
