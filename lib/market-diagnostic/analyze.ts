import type { NormPlace, RawSnapshot } from "./fetch";

/**
 * Market-diagnostic analysis layer (server port of scripts/market-diagnostic/analyze-diagnostic.mjs).
 * Classifies every place with Claude Haiku, then computes the consulting-grade diagnostic:
 * competitor landscape (share-of-voice), prioritized referral call-sheet, channel playbook.
 *
 * Output shape is IDENTICAL to the committed *.analysis.json the UI already renders — do not
 * change field names without updating components/provider/market/MarketDiagnostic.tsx.
 */

const ANTHROPIC_KEY = () => process.env.ANTHROPIC_API_KEY;
const MODEL = "claude-haiku-4-5";
const MAXMI = 25; // local catchment cap (Places bias isn't a hard limit)
...
interface Label { i: number; cat: string; referralValue: string }

async function classifyBatch(places: NormPlace[]): Promise<Label[]> {
...
type Classified = NormPlace & { cat: string; referralValue: string; q?: number };

export interface MarketAnalysis {
  meta: RawSnapshot["meta"];
  demand: { demographics: unknown; olera: Record<string, unknown> };
  competitorLandscape: unknown;
  referralGraph: unknown;
  channels: unknown[];
  classificationCounts: { cat: string; n: number }[];
}

/** Run the full analysis on a raw snapshot. Returns the analysis object + Haiku token usage est. */
export async function analyzeSnapshot(snap: RawSnapshot): Promise<{ analysis: MarketAnalysis; classified: number }> {
  // Single deduped universe (competitors + referralGraph)
  ...
  const NAME_RULES: [RegExp, string, string][] = [
  ...
  const inCatchment = (p: Classified) => p.distanceMiles == null || p.distanceMiles <= MAXMI;
  ...
  const ROLE_VALUE: Record<string, number> = { hospital: 7, skilled_nursing: 6, hospice: 5, assisted_living: 4, elder_law: 3, senior_resource: 3, home_health: 2, financial: 1, faith: 1 };
  ...
  const byRole: Record<string, Classified[]> = {};
  ...
  const PER_CAT: Record<string, number> = { hospital: 4, skilled_nursing: 3, hospice: 3, assisted_living: 4, elder_law: 4, senior_resource: 2 };
  const isReal = (r: Classified) => (r.reviews > 0 || r.phone) && !/meditech|\(system\)/i.test(r.name || "");
  const picks: Classified[][] = BD_CATS.map((cat) => (byRole[cat] || []).filter(isReal)
    .map((r) => ({ ...r, q: Math.min((r.reviews || 0) / 100, 2) - (r.distanceMiles || 30) * 0.05 }))
    .sort((a, b) => (b.q || 0) - (a.q || 0)).slice(0, PER_CAT[cat] || 3));
  const bdTargets: Classified[] = [];
  ...
  const channels = [
    ...
    { channel: "Google LSA / SEO / targeted PPC", priority: 4, key: "ads",
      rationale: `Real, but volume-capped in a senior-thin market. The smart play is Local Services Ads + tight geo — not broad spend.`,
  ...
  const analysis: MarketAnalysis = {
    meta: snap.meta,
    demand: {
      demographics: (snap.demographics as { totals?: { population?: number } })?.totals?.population
        ? snap.demographics
        : { note: "Census demographics unavailable for this catchment (no ZCTA match or CENSUS_API_KEY unset)" },
  ...
  return { analysis, classified: universe.length };
}
