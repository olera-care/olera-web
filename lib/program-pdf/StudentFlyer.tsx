...
/**
 * The student flyer: one page, addressed to the student rather than the
 * agency.
 *
 * This is the layout both audiences used to share. When the provider
 * brochure was redesigned around its own content — a price, a team, a
 * one-word reply — the student flyer inherited all of it, and for one commit
 * it told students their first hire was free and cost $250 after that.
 *
 * So the two are separate documents now. They share the assets, the palette
 * and nothing else, because they are not addressed to the same person.
 */
  ...
  return (
    ...
    >
      <Page size="LETTER" style={styles.page}>
          ...
          {config.steps.map((s, i) => (
            <View key={i} style={styles.stepCell}>
              <View style={styles.stepInner}>
                ...
                {/* The provider brochure gives a step a lead and a
                    sentence; a student step is one clause. */}
                <Text style={styles.stepText}>
                  {typeof s === "string" ? s : `${s.title} ${s.body}`}
                </Text>
              </View>
            </View>
          ))}
        ...
        {/* ── Participation / what to expect. Optional since the
            provider brochure dropped its price panel for an offer. */}
        {config.pricing ? (
          <>
