...
export const SMS_VARIANTS: SmsVariant[] = [
  ...
  // ─────────────── Family · Benefits cascade (coordinator B1/B2 mirrors) ───────────────
  {
    ...
    label: "First step (day 2-3) — B1 mirror",
    emailType: "benefits_first_step_sms",
    trigger: "The coordinator's B1 rung fires (intake +48-96h) and the family has a stored phone + sms_consent — the text rides alongside the ten-minute-first-step email",
    who: "A benefits-intake family in the B1 band who gave us their number and tapped the text consent ask (enrichment step 4 or the wants-help page). Consent is required — a phone alone never gets this.",
    why: "The email's ten-minute first step, compressed to one segment: the program, the number to call, the documents to have nearby, and the guide link. 90%+ SMS open rates reach the families whose email sits unread.",
    gates: [
      ...
      "Only sends when the B1 email itself sends — the mirror never fires standalone",
