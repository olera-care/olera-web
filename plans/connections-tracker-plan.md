...
- [x] 6. Sidebar link under **Inbox** ("Connections", after Demand). Empty/loading/error states present.
- [x] 7. **Made rows actionable** (added 2026-06-01 after TJ flagged the view was display-only): rows expand on click → lazy-fetch `GET /api/admin/connections/[id]` (the gap: family's ask, conversation thread tagged by role + auto-reply, provider email + engagement, nudge history) → **recommended next step + one-click action**. `recommendNextStep()` in `connection-temperature.ts`: nudge_provider (wired to existing `POST /api/admin/send-nudge`, with cooldown/no-email/error feedback), add_provider_email (→ Leads needs-email), follow_up_family (advice; one-click pending sub-task 2). Verified next-step + thread-role logic on real data.
