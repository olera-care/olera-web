...
function parseArgs() {
  ...
  const opts = { state: null, region: null, parentState: null, phase: "all", run: false, mode: "seed" };
...
9. NEVER cite Olera as a source. If a fact cannot be verified from .gov or official program sources, say so.`;

// Few-shot exemplars from Texas editorial audit (Chantel's quality bar)
const QUALITY_EXEMPLARS = `
EXEMPLAR — Document Checklists (MEPD, 15 items, note the granularity):
...
EXEMPLAR — FAQs (real decision-making questions, not definitions):
...
Q: "Can a family member apply on behalf of an elderly parent?" A: "Yes. You can apply as an authorized representative by completing Form H1003 (Authorized Representative designation). Bring your own ID and your parent's documents."
Q: "What if my landlord won't sign the permission form?" A: "If your landlord refuses, contact your local weatherization agency — they may be able to negotiate directly. You cannot proceed without landlord consent for rental properties."

EXEMPLAR — Application notes (conditional guidance):
["Crisis cases with active disconnection notices may get expedited processing within 48 hours", "Some MCOs have immediate openings while others maintain 6-month waitlists — ask about availability when choosing", "You can apply while still in the hospital; discharge planners often initiate the process"]

EXEMPLAR — Contacts:
[{"label": "Texas 2-1-1", "phone": "2-1-1", "description": "Free 24/7 helpline for all social services", "hours": "24 hours, 7 days a week"}, {"label": "HHSC Benefits Line", "phone": "(877) 541-7905", "description": "Medicaid and benefits questions", "hours": "Mon-Fri 8am-6pm CT"}]
`;
...
async function phaseDraft(entity, diveData, classifyData) {
  ...
  const results = await runConcurrent(divePrograms, async (prog, i) => {
    ...
    const allProgramNames = divePrograms.map((p) => p.name).filter((n) => n !== name);
...
Other programs in this state (for cross-references): ${allProgramNames.join(", ") || "none yet"}.
...

{
  ...
  "documentsNeeded": [
    <6-15 program-specific documents. Be CONCRETE: "Social Security award letter" not "proof of income". Include conditional notes in parentheses, e.g., "Landlord verification form (provided by caseworker at interview)". See MEPD exemplar above for the quality bar. Simple programs (resources) can have 0-3 items.>
  ],

  "contacts": [
    <Phone numbers, hotlines, TTY lines. Each: {"label": "...", "phone": "...", "hours": "...", "description": "..."}. Include state 2-1-1 and program-specific lines. See exemplar above.>
  ],

  "regionalApplications": [
    <If program has regional intake offices or area-specific application forms, list each: {"region": "...", "counties": [...], "url": "...", "isPdf": boolean}. Null/empty if statewide single intake.>
  ],

  "applicationNotes": [
    <Conditional guidance strings: crisis expediting, waitlist realities, tips that depend on situation. See exemplar above. 2-4 notes for complex programs, 0-1 for simple.>
  ],

  "relatedPrograms": [
    <Names of other programs in this state a caregiver looking at THIS program should also know about. Use the program names from the list above.>
  ],

  "faqs": [
    <${complexity === "deep" ? "6-8" : complexity === "medium" ? "4-6" : "2-4"} FAQs that answer REAL DECISION-MAKING questions. Not "What is this program?" but "Can Mom keep her house if she qualifies?", "Can a family member apply on behalf of a parent?", "What happens if income is just over the limit?", "Can I apply for X and Y at the same time?". Must include at least one cross-program FAQ if related programs exist. See exemplar above for quality bar. Format: {"question": "...", "answer": "..."}>
  ],
...
- NEVER cite Olera or olera.com as a source. Only .gov, official program, or established organization URLs.
- Documents needed must be program-specific and concrete. "Social Security award letter" not "proof of income".
- Contacts must include actual phone numbers from research. Do not fabricate phone numbers.
- Related programs must use names from the provided list of other programs in this state.
...
// ═══════════════════════════════════════════════════════════════════════════
// MODE: REFINE — Re-draft weak programs from existing drafts
// ═══════════════════════════════════════════════════════════════════════════

async function modeRefine(entity, existingDrafts) {
  console.log(`\n  ━━━ REFINE: Improving weak drafts ━━━\n`);

  if (!ANTHROPIC_API_KEY) {
    console.log(`  WARN: ANTHROPIC_API_KEY not set. Cannot refine.`);
    return existingDrafts;
  }

  const programs = existingDrafts?.programs?.filter((p) => !p._error && !p._parseError) || [];
  if (!programs.length) {
    console.log(`  No existing drafts to refine.`);
    return existingDrafts;
  }

  // Identify weak programs
  const weak = programs.filter((p) => {
    const weaknesses = [];
    if (!p.documentsNeeded || p.documentsNeeded.length < 3) weaknesses.push("documents");
    if (!p.faqs || p.faqs.length < 4) weaknesses.push("faqs");
    if (!p.contacts || p.contacts.length === 0) weaknesses.push("contacts");
    if (!p.applicationNotes || p.applicationNotes.length === 0) weaknesses.push("applicationNotes");
    if (!p.relatedPrograms || p.relatedPrograms.length === 0) weaknesses.push("relatedPrograms");
    p._weaknesses = weaknesses;
    return weaknesses.length > 0;
  });

  if (!weak.length) {
    console.log(`  All ${programs.length} drafts meet quality bar. Nothing to refine.`);
    return existingDrafts;
  }

  console.log(`  ${weak.length}/${programs.length} programs need refinement:\n`);
  for (const p of weak) {
    console.log(`    ${p.name}: missing ${p._weaknesses.join(", ")}`);
  }
  ...
  for (const prog of weak) {
    const refinePrompt = `${CONTENT_VOICE_PROMPT}

${QUALITY_EXEMPLARS}

You have an existing draft for "${prog.name}" in ${entity.name}. It needs improvement in these areas: ${prog._weaknesses.join(", ")}.

Other programs in this state: ${allProgramNames.filter((n) => n !== prog.name).join(", ")}

Current draft:
${JSON.stringify({ ...prog, _weaknesses: undefined }, null, 2)}

Improve ONLY the weak areas. Return the COMPLETE updated JSON object (all fields, not just the changed ones). Keep all existing good content unchanged. Follow the quality exemplars above for the fields you're improving.

CRITICAL: Return ONLY the JSON object, no markdown wrapping.`;

    try {
      await claudeLimiter.wait();
      const response = await claudeChat(refinePrompt, 6144);
      const parsed = extractJson(response);

      if (parsed) {
        // Merge refined fields back into existing draft
        const idx = programs.indexOf(prog);
        const merged = { ...prog, ...parsed, _weaknesses: undefined, contentStatus: "pipeline-draft", draftedAt };
        programs[idx] = merged;
        refined++;
        process.stdout.write(`  [${refined}/${weak.length}] Refined: ${prog.name.slice(0, 50).padEnd(50)}\r`);
      } else {
        console.log(`\n  WARN: Could not parse refined draft for "${prog.name}"`);
      }
    } catch (err) {
      console.log(`\n  ERROR refining "${prog.name}": ${err.message}`);
    }
  }

  console.log(`\n\n  Refined ${refined}/${weak.length} programs`);

  // Clean up internal _weaknesses field from all programs
  for (const p of programs) {
    delete p._weaknesses;
  }
...
// ═══════════════════════════════════════════════════════════════════════════
// MODE: UPDATE — Check for changed data without full re-draft
// ═══════════════════════════════════════════════════════════════════════════

async function modeUpdate(entity, existingDrafts) {
  console.log(`\n  ━━━ UPDATE: Checking for data changes ━━━\n`);

  // Re-run explore + dive to get fresh data
  const exploreData = await phaseExplore(entity);
  const diveData = await phaseDive(entity, exploreData);

  if (!diveData?.programs?.length) {
    console.log(`  No dive data. Cannot update.`);
    return existingDrafts;
  }

  const existingPrograms = existingDrafts?.programs?.filter((p) => !p._error && !p._parseError) || [];
  let patchCount = 0;

  // For each existing program, check if dive data has changed values
  for (const existing of existingPrograms) {
    const match = diveData.programs.find((d) =>
      d.name?.toLowerCase().includes(existing.shortName?.toLowerCase()) ||
      existing.name?.toLowerCase().includes(d.name?.toLowerCase().split(" ")[0])
    );
    if (!match) continue;

    // Check for changed phone numbers, URLs, or key facts
    if (match.application?.phone && match.application.phone !== existing.phone) {
      console.log(`  PATCH: ${existing.name} phone ${existing.phone} → ${match.application.phone}`);
      existing.phone = match.application.phone;
      patchCount++;
    }
    if (match.application?.url && match.application.url !== existing.sourceUrl) {
      console.log(`  PATCH: ${existing.name} sourceUrl updated`);
      existing.sourceUrl = match.application.url;
      patchCount++;
    }
  }

  console.log(`\n  Patched ${patchCount} fields across ${existingPrograms.length} programs`);
...
async function main() {
  ...
  console.log(`  Pipeline mode: ${opts.mode}`);
  console.log(`  Execution:     ${opts.run ? "EXECUTE" : "DRY RUN"}`);
  ...
  if (opts.mode === "refine") {
    const existingDrafts = readJson(dirName, "drafts.json");
    if (!existingDrafts?.programs?.length) {
      console.log(`\n  No existing drafts to refine. Run seed mode first.`);
      process.exit(1);
    }
    const refinedDrafts = await modeRefine(entity, existingDrafts);
    writeFile(dirName, "drafts.json", refinedDrafts);
  ...
  if (opts.mode === "update") {
    const existingDrafts = readJson(dirName, "drafts.json");
    if (!existingDrafts?.programs?.length) {
      console.log(`\n  No existing drafts to update. Run seed mode first.`);
      process.exit(1);
    }
    const updatedDrafts = await modeUpdate(entity, existingDrafts);
    writeFile(dirName, "drafts.json", updatedDrafts);
    generatePipelineSummary();
    generatePipelineDrafts();
    console.log(`\n  Done. ${cost.summary()}\n`);
    return;
