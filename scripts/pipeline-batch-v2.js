...
async function phaseClean(cities, opts) {
  ...
  // v2: Pooled classify (non-watch) vs per-city streaming (watch)
  if (opts.watch) {
...
async function phaseLoad(cities, opts) {
  ...
  // v2: fire-and-forget live site verification (skip on dry-run)
  if (!opts.dryRun) {
    opts._liveSiteCheckPromise = runLiveSiteCheck(cities);
  }
}

// v2: Live site verification hook — picks 5 random cities with >= 3 loaded providers
...
async function phaseFinalize(cities, opts) {
  ...
  // v2: await + print live site verification results
...
async function main() {
  ...
  console.log(`\n${'='.repeat(70)}`);
  console.log('OLERA CITY PIPELINE — BATCH PROCESSOR v2 (pooled classify + streaming + live check)');
  console.log(`${'='.repeat(70)}`);
  console.log(`  Cities:       ${cities.length}`);
  console.log(`  Phase:        ${opts.phase}`);
  console.log(`  Expansion:    ${opts.expansionDir}`);
  console.log(`  Resume:       ${opts.resume}`);
  console.log(`  Est. cost:    ~$${estCost} (post-discovery, v2)`);
  console.log(`  Est. time:    ~${formatDuration(estTimeMin * 60)} (v2 pooled classify + streaming + live check)`);
