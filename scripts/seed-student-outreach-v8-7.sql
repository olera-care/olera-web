-- ─────────────────────────────────────────────────────────────────────────
-- Student Outreach v8.7 demo seed — Texas A&M + University of Houston.
--
...
--
-- Fourteen stakeholders, scoped to two campuses, hitting every workflow
-- the v8.7 panel surfaces:
--
--   Research tab
--     Campus cards (TAMU + UH have stakeholders in prospect/researched)
...
--
--   Calls tab
--     8. UH Pre-Health Advising              — advisor, overdue follow-up call
--
--   Replies tab
--     Needs attention
--       4. TAMU Health Professions Office    — wants_meeting (Replies + Meetings/in_flight)
--       9. UH Biology Department             — engaged (replied yesterday)
...
--
--   Meetings tab
--      4. TAMU Health Professions Office     — finding a time (in_flight)
--      5. TAMU Chemistry Department          — booked tomorrow
--
--   Active Partners tab
--      6. TAMU Pre-Health Honor Society      — active_partner
...
BEGIN
  ...
  -- Overdue follow-up call task (puts the row on the Calls tab).
  INSERT INTO student_outreach_tasks (
    outreach_id, task_type, due_at, payload, status, created_by
  ) VALUES (
  ...
  -- ── 9. UH Biology Department — dept_head — engaged ─────────────────────
  ...
  RAISE NOTICE 'Seeded 14 student-outreach demo rows across Texas A&M (%) and University of Houston (%).', tamu_id, uh_id;
END $$;
