-- Migration: Provider connection status self-report event
--
-- Adds 'provider_connection_status_reported' to provider_activity event types.
-- This event is logged when a provider self-reports their connection status
-- via email buttons (connected, not_a_fit, no_capacity).
...
ALTER TABLE provider_activity ADD CONSTRAINT provider_activity_event_type_check CHECK (
  event_type IN (
    ...
    'provider_connection_status_reported'
  )
);

COMMENT ON CONSTRAINT provider_activity_event_type_check ON provider_activity IS
'Allowed event types including provider connection status self-report. Last update: migration 186 (provider_connection_status_event).';
