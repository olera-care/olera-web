-- Notification settings updates and their outcome events commit together.
-- Apply before deploying the notification settings API. Existing event values
-- are preserved, including values added by migrations in other feature branches.
BEGIN;
DO $$
...
BEGIN
  ...
  EXECUTE 'ALTER TABLE public.provider_activity ADD CONSTRAINT provider_activity_event_type_check CHECK (' ||
    ...
    ' OR event_type IN (''notification_settings_viewed'', ''notification_preference_saved''))';
END $$;
