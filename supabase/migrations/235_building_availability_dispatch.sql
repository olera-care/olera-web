-- Atomic reservation for the building availability email.
-- Same pattern as reserve_building_payment_insurance: one durable attempt per provider,
-- a worker crash leaves a visible pending row, never a resend loop.
BEGIN;
CREATE OR REPLACE FUNCTION public.reserve_building_availability(p_profile_id uuid, p_email text)
...
BEGIN
  ...
  IF NOT FOUND OR p.type <> 'organization' OR p.account_id IS NULL OR p.email IS NULL
    ...
    OR p.metadata->>'building_availability_attempt_id' IS NOT NULL
  ...
  -- as the payment & insurance email's digest deferral.
  ...
  VALUES (p.email, 'Are you accepting new clients?', 'building_availability', 'provider', p.id::text, 'pending')
  RETURNING id INTO log_id;
  UPDATE business_profiles SET metadata = COALESCE(metadata, '{}'::jsonb) ||
    jsonb_build_object('building_availability_attempt_id', log_id, 'building_availability_attempted_at', now())
    WHERE id = p_profile_id;
  RETURN log_id;
END $$;
REVOKE ALL ON FUNCTION public.reserve_building_availability(uuid,text) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.reserve_building_availability(uuid,text) TO service_role;

-- Starts paused until copy QA and staging dry run are complete.
INSERT INTO cron_config (job_id, enabled, paused_reason)
VALUES ('building-availability', false, 'Awaiting copy QA and staging dry run')
